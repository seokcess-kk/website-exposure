...
session id: 019e2998-f536-7273-972b-5a38b7839a74
--------
user
...
codex
요청하신 형식으로 v0.3 acceptance 검증을 하겠습니다. 먼저 대상 문서와 cascade 대상 SoT를 읽고, 이전 응답에서 수용했다는 항목들이 실제로 반영됐는지 대조하겠습니다.
exec
...
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'Get-Content -Raw .codex-reviews/spike_cycle2_response.md' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'Get-Content -Raw docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md' in C:\Users\assag\solution\website-exposure
 succeeded in 690ms:
# Phase 0 Week 1~2 Spike 계획 — 5개 가설 검증 (v0.3 — codex 2차 비평 7 지적 전건 수용)
...
|---|---|---|---|
| A·D PROVIDER_PASS + B·C·E LOCAL_PASS | Week 3 schema/migration·worker skeleton·UI 컴포넌트 | Week 4 production-readiness·Storage 적용·admin login flow until provider gate | minor |
...
reasoning effort: none
reasoning summaries: none
session id: 019e2993-dc47-74a1-9ba8-ac4c30a461de
--------
user
...
- `C:\Users\assag\solution\website-exposure\docs\core\DATA_MODEL.md`
codex
대상 문서와 관련 SoT를 먼저 대조해서 재발 여부와 새 모순을 분리해 보겠습니다. 읽기 작업은 병렬로 진행해 문서 간 cascade 차이를 먼저 잡겠습니다.
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'Get-Content -Raw .codex-reviews/spike_cycle1_response.md' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'Get-Content -Raw docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'Get-Content -Raw docs/core/DATA_MODEL.md' in C:\Users\assag\solution\website-exposure
...
type AuditAction =
  ...
  | "service-role-invoked"                    // INFRA2-02 — service_role break-glass 사용 추적
  | "instance-switched"                       // INFRA2-08 — super-admin cross-instance 전환
  // Spike 결정 cascade (SPIKE1-13)
  | "signed-url-issued"                       // SPIKE1-13 — R2 signed URL 발급 (signed URL 자체는 audit. URL body는 미저장)
  | "signed-url-revoked";                     // SPIKE1-13 — signed URL TTL 만료 전 강제 무효화 (운영 incident 대응)
```
...
| 2026-05-14 | v0.2 | **codex 자동 비평 1차 반영 (12개 지적 전건 수용)**: (1)·(2) § 2.3 상태 전이 완전화 — `blocked → draft`·`rejected → draft`/`review-queued` 분리·`request-changes` 전이·`published → blocked` 사후 fail·`published → stale` 우선순위 추가, (3) § 3.1.1 warning 큐 이탈 조건·기록 슬롯 신설 (acknowledged·resolved). § 7.1 (6) publishable 조건 추가, (4) § 4.1 AND 게이트 평가 알고리즘 정밀화 — priorReview·LegalDocument legal 자동 추가 + approved vs publishable 시점 분리 명시, (5) § 4.1 riskLevel 출처 명시 — `ComplianceRecord.pageRiskLevel` (RiskInference MAX 결합 결과), (6) § 7.1 LegalDocument 조건 — `legalCounsel` + `legalCounselAt` 둘 다 필수. 각 역할 매핑 timestamp 필드도 모두 명시, (7) § 5.2 ComplianceRecord 생명주기 2단계 분리 — pre-publish(mutable) vs published(immutable). C-10 required 필드 충돌 해소(AW-10), (8) § 5.4 staleFlags를 별도 `StaleFlagsRegistry` 컬렉션으로 분리 — published record 불변성 보장(AW-11), (9) § 6.2 stale 처리 흐름 명확화 — published 표면 유지·재발행 명시 액션 필요·이전 record audit log 보존, (10) § 4.1·§ 8 사전심의와 publishable 결합 명시 — `priorReviewRequired=true` 시 finalRoles에 legal 자동 추가, (11) § 3.1·§ 9.1 content-gate 큐 처리자·알림 수신자를 `finalRoles[]` 기준으로 정정 — operator·등급 기본 medical 포함, (12) § 11.2 super-admin 자격 우회 금지 — medical/legal/client approve 시 RISK_LEVELS § 4 자격 검증 필수 |
 ...
 succeeded in 1325ms:
# Phase 0 Week 1~2 Spike 계획 — 5개 가설 검증 (v0.2 — codex 1차 비평 18 지적 전건 수용)
...
| Day 8 (Mon Week 2) | C-provider — R2 staging IAM·credential·real presigned URL | provider smoke gate |
| Day 9 (Tue Week 2) | A-provider — Supabase Pooler transaction mode + RLS smoke | provider smoke gate |
| Day 10 (Wed Week 2) | 종합 보고서·v1.0 결정 갱신·Week 3~6 계획 분기 | dependency graph 결과 반영 |
...
### 0.4 신규 AuditAction cascade (SPIKE1-13)

REVIEW_WORKFLOW § 10.2.1에 cascade 완료:
- `signed-url-issued` — R2 signed URL 발급 추적
- `signed-url-revoked` — TTL 만료 전 강제 무효화
...
| 실패 항목 | fallback | reversal blast radius |
|---|---|---|
| pgBouncer SET LOCAL leak | Supabase Pooler 사용 또는 direct connection·또는 connection-scoped role | 비용·확장성 trade-off. INFRA2-01 일부 변경 |
| Drizzle SET LOCAL 비호환 | Kysely 또는 raw SQL adapter | `packages/db` 전면 재작성·INFRA 결정 v1.0 §2 stack 일부 reversal |
| RLS overhead p95 > 50ms | application-level scoping 주력·RLS 보조만 | INFRA2-01 전체 reversal — INFRA 결정 v1.0 §1.1 재작성·1차 결정으로 reversal |
| service_role 격리 안 됨 | env 분리 강제·연결 string 분리 | runtime guard 강화 |
...
## 종합 dependency graph (SPIKE1-15)

```
Day 1-2  A PASS ──┬─→ Week 3-4 schema migration·scopedDb·composite FK
                  │
Day 3-4  B PASS ──┼─→ Week 4-5 outbox base·worker skeleton·notifications dispatch
                  │
Day 5    C-local PASS ──┐
Day 8    C-provider PASS ──┴─→ Week 5+ packages/storage·blobRef·feature blob 적용
                  │
Day 6    D PASS ──┴─→ Week 4 M0 vertical slice migration 진행
                  │
Day 7    E PASS ──┴─→ Week 6 admin UI middleware·session·login flow

Day 9    A-provider PASS (Supabase Pooler) ──→ A 결정 v1.0 확정
Day 10   종합 보고서·dependency 확인·Week 3~6 계획 분기
```

각 Spike 실패 시:
- A FAIL → 전체 Phase 0 scope 재산정 (Week 2 Day 10 gate)
- B FAIL → worker 패턴 재설계·notifications P0 subset 재정의
- C FAIL → Storage ADR 변경·Phase 1+ 영향
- D FAIL → migration tooling 재선택
- E FAIL → Auth provider 변경
...
codex
1차 지적은 대부분 문서상 반영되어 있습니다. 지금 남는 리스크는 “반영 여부”보다, 10일 계획이 인프라 SoT의 Week 1/Week 2 구분과 어떻게 충돌하는지, 그리고 D/E를 하루 단위로 자를 때 pass/fail 산출물이 실제로 남는지 쪽입니다.
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'rg -n "Day 1|provider smoke gate|Buffer|Spike D|Spike E|dependency graph|reversal blast radius|signed-url-issued|signed-url-revoked" docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'rg -n "Phase 0 Spike Gate|Phase 0 작업|Week 1|Week 2|Spike A|Spike B|Spike C" docs/decisions/INFRA_DECISIONS_DRAFT.md' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'rg -n "signed-url-issued|signed-url-revoked|service-role-invoked|instance-switched" docs/admin/REVIEW_WORKFLOW.md' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'rg -n "instanceMemberships|active|role|approverRoleEligibility|eligibilityEvidence" docs/core/DATA_MODEL.md' in C:\Users\assag\solution\website-exposure
 succeeded in 828ms:
294:## 영역 4: Phase 0 작업 + 베타·법무 일정
296:### 4.1 Phase 0 Spike Gate (INFRA2-09 신규)
298:Week 1에 다음 3개 spike를 별도 gate로 진행:
302:| **Spike A: Drizzle + RLS + Auth.js + Supabase** | tenant scoping 실행 모델 검증 | (1) withTenantTransaction 안에서 RLS 적용 (2) 다른 instance row 안 보임 (3) pgBouncer transaction pooling 작동 (4) lint·runtime guard 동작 | Phase 0 scope를 UI보다 infra closure로 재조정·migration 패턴 재설계 |
303:| **Spike B: worker control-plane queue + tenant processing** | SKIP LOCKED claim·instance switch 검증 | (1) control-plane DB role로 claim·(2) tenant-plane으로 처리·(3) worker가 instance context를 RLS에 set·(4) 두 transaction 분리 | worker 패턴 재설계 |
304:| **Spike C: Storage signed URL isolation** | (B Storage 채택 시) R2 signed URL + instance prefix·IAM condition·next-auth 매핑 검증 | (1) cross-instance access 차단·(2) signed URL refresh 동작·(3) IAM PolicyDocument 작동 | Storage ADR 재검토 |
306:Spike 통과 후 Week 2~6 본 작업 진행. 실패 시 Phase 0 일정 4주 buffer.
308:### 4.2 Phase 0 작업 (v0.2 § 3 + spike 추가)
312:| **Week 1 (Spike)** | Spike A·B·C 진행 + repo skeleton + tsconfig + Biome + CI 기본 | Spike 통과 |
313:| Week 2 | Supabase dev/staging·Drizzle migration·next-auth·Sentry·Resend dev·Upstash | dev/staging vertical green |
355:| Phase 0 (Week 1~6) | **Fixed-scope package** — DPA 템플릿 1회·의료광고 워크플로 2회 워크숍·high-risk sample 20건 검토·사전심의 절차 자문 1회 | 500~1500만원 (1회성) |
427:| 4-1. Phase 0 | 6~8주·Week 1 Spike A·B·C gate·M0 vertical slice schema·P0 outbox subset (notifications 4 tables 포함) |
453:| 2026-05-15 | (v0.3 비고 이전) | **codex 2차 15 지적 전건 수용 + cascade**: (1) **RLS 실행 모델** — withTenantTransaction 헬퍼·SET LOCAL·worker control/tenant plane 분리·pgBouncer transaction pooling·lint·runtime guard (INFRA2-01), (2) **REVIEW_WORKFLOW cascade — service-role-invoked·instance-switched AuditAction 2종 추가** (INFRA2-02·08), (3) **Phase 0 outbox 옵션 A** — P0에 notifications 최소 subset (Receipt·Log·PayloadRecord·DeliveryAttempt) 포함 (INFRA2-03), (4) **composite FK 3등급 분류** — tenant-plane hard FK·control-plane FK·polymorphic ref typed registry (INFRA2-04), (5) **tenant export/import manifest dependency class** — portable·rebind-required·rotate-required·legal-reapproval-required·external-provider-owned·blob-copy-required·audit-chain-preserved (INFRA2-05), (6) **rate limit taxonomy** — Postgres hard quota·Redis soft cache 분리 (INFRA2-06), (7) **Storage ADR — Cloudflare R2 reversal 권장** (INFRA2-07), (8) **resolveTenantContext** — server-side membership/role/legal eligibility 검증·instance-switched audit (INFRA2-08), (9) **Spike A·B·C gate Week 1** (INFRA2-09), (10) **legal-reviewer fixed-scope package → 시간당 → retainer 단계** (INFRA2-10), (11) **internal beta는 workflow technical validation 한정** (INFRA2-11), (12) **customer domain ADR 별도** (INFRA2-12), (13) **사전심의 manual-assisted workflow** — submission packet export·institutionType enum (INFRA2-13), (14) **PIPA + GDPR checklist** Phase 1 gate (INFRA2-14), (15) **DATA_MODEL C-08 v0.23 cascade — email transport/provider 분리** (INFRA2-15) |
...
974:| `instanceMemberships` | `Array<{instanceId: Slug, role: AdminUserRole, joinedAt: Date}>` | ✅ | 사용자가 접근 가능한 인스턴스 목록 (multi-tenant) |
...
1107:| 2026-05-14 | v0.13 | **`features/notifications.md` cascade (1차+3차 사이클 통합)**: (1) **C-08 확장** — `adminBaseUrl`(URL, notifications 활성 시 required) + `timezone`(IANATimezone, notifications·SLA 활성 시 required) + `notificationChannels`를 `NotificationChannelsConfig`로 확장(email transport·secretRef·sender·rateLimit / slack webhookUrlSecretRef·rateLimit / inApp) + **`holidayCalendar`(region·source — 3차 cycle N3-13)**, (2) **C-23 `AdminUser` 신설** — 어드민 사용자·자격·알림 선호 SoT. `id`·`email`·`role`(AdminUserRole)·`approverRoleEligibility[]`·`eligibilityEvidence[]`·`slackUserId`·`timezone`(quietHours 한정 — 3차 cycle N3-20)·`notificationPreferences`(channels·digestOptOut·quietHours·**suppression with autoReleaseAt** — 3차 cycle N3-15)·`instanceMemberships[]`·`active`, (3) **`IANATimezone` 공통 타입 표기** (IANA Time Zone Database 식별자), (4) 인벤토리 22개 → 23개 |

 succeeded in 827ms:
708:  | "service-role-invoked"                    // INFRA2-02 — service_role break-glass 사용 추적
709:  | "instance-switched"                       // INFRA2-08 — super-admin cross-instance 전환
711:  | "signed-url-issued"                       // SPIKE1-13 — R2 signed URL 발급 (signed URL 자체는 audit. URL body는 미저장)
712:  | "signed-url-revoked";                     // SPIKE1-13 — signed URL TTL 만료 전 강제 무효화 (운영 incident 대응)

 succeeded in 874ms:
6:> **목적**: Phase 0 Week 1~2 안에 가장 위험한 기술 가정 5개 (A·B·C·D·E) 검증. provider smoke gate 포함. 통과 시 본 구현 진입·실패 시 Phase 0 scope 재조정
7:> **핵심 변경 (v0.2)**: 5 Spike 확장 (D·E 추가)·provider smoke gate·exactly-once → idempotent at-least-once with exactly-once observable effects·dependency graph·invariant 반복 검증·signed-url-issued AuditAction cascade
17:| Day 1 (Mon) | A — DB·RLS 검증 | C-local 병행 가능 |
24:| Day 8 (Mon Week 2) | C-provider — R2 staging IAM·credential·real presigned URL | provider smoke gate |
25:| Day 9 (Tue Week 2) | A-provider — Supabase Pooler transaction mode + RLS smoke | provider smoke gate |
26:| Day 10 (Wed Week 2) | 종합 보고서·v1.0 결정 갱신·Week 3~6 계획 분기 | dependency graph 결과 반영 |
28:Buffer: Week 2 Day 11~14는 본 작업 (Phase 0 § 4.2 v0.3 일정).
33:- **가설 (hypothesis)**·**실험 (experiment)**·**pass criteria**·**negative invariant 측정 방법**·**provider smoke 분리**·**실패 시 fallback + reversal blast radius**·**downstream unblock**
41:### 0.3 provider smoke gate (SPIKE1-01·04 — 핵심 정정)
44:- **local PASS**: docker-compose 환경에서 통과 (Day 1~7)
52:- `signed-url-issued` — R2 signed URL 발급 추적
53:- `signed-url-revoked` — TTL 만료 전 강제 무효화
57:## Spike A: Drizzle + RLS + tenant scoping (DB-only, Auth.js는 Spike E)
63:(Auth.js·resolveTenantContext는 Spike E로 분리 — SPIKE1-03·18)
117:### A.4 실패 시 대안 + reversal blast radius (SPIKE1-14)
119:| 실패 항목 | fallback | reversal blast radius |
129:- A FAIL → Phase 0 scope 재산정 gate (Week 2 Day 10에 의사결정)
180:| 실패 | fallback | reversal blast radius |
228:### C.4-provider: R2 staging IAM·real presigned (Day 8 — provider smoke gate)
235:| cross-instance copy (import) | service_role만 가능 + `signed-url-issued` audit |
239:| 실패 | fallback | reversal blast radius |
253:## Spike D: Drizzle Kit migration deploy (SPIKE1-02 신규 — P0)
299:| 실패 | fallback | reversal blast radius |
312:## Spike E: Auth.js + resolveTenantContext + membership (SPIKE1-03 신규 — P0)
357:| 실패 | fallback | reversal blast radius |
382:## 종합 dependency graph (SPIKE1-15)
385:Day 1-2  A PASS ──┬─→ Week 3-4 schema migration·scopedDb·composite FK
397:Day 10   종합 보고서·dependency 확인·Week 3~6 계획 분기
401:- A FAIL → 전체 Phase 0 scope 재산정 (Week 2 Day 10 gate)
414:| 2026-05-15 | **v0.2** | **codex 1차 비평 18 지적 전건 수용**: (1) **provider smoke gate** — local + provider 2단계 분리 (SPIKE1-01·04), (2) **Spike D 신설** — Drizzle Kit migration deploy (SPIKE1-02), (3) **Spike E 신설** — Auth.js + resolveTenantContext + membership invalidation (SPIKE1-03), (4) **invariant 측정 표준화** — 1000 iterations × 20 concurrent + invariant SQL binary 검증 (SPIKE1-05), (5) **A 쓰기 path·rollback·nested tx·audit_log RLS 시나리오 추가** (SPIKE1-06·07), (6) **B 가설 정정** — idempotent at-least-once with exactly-once observable effects (SPIKE1-08), (7) **B failure injection 8 point + 외부 call count invariant** (SPIKE1-09·10), (8) **C TTL 상태 코드 정정** — 401 → 403 (provider semantics·SPIKE1-11), (9) **C security 시나리오 확장** — replay·method confusion·ListBucket·content-type·URL scrubbing (SPIKE1-12), (10) **REVIEW_WORKFLOW cascade** — `signed-url-issued`·`signed-url-revoked` AuditAction 2종 추가 (SPIKE1-13), (11) **fallback에 reversal blast radius 명시** (SPIKE1-14), (12) **Week 3~6 dependency graph** (SPIKE1-15), (13) **일정 7일 → 10일** (Week 1 + Week 2 Day 8~10·SPIKE1-16), (14) **성능 기준 correctness와 분리** (SPIKE1-17), (15) **A/B scope 명확화** — A는 transaction/RLS·B는 queue/crash (SPIKE1-18), (16) Spike F·G P1·후속 명시 |

exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'rg -n "membership active=false|active=false|instanceMembership|membership.active|requestedInstanceId|session refresh" docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md docs/decisions/INFRA_DECISIONS_DRAFT.md docs/core/DATA_MODEL.md' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'rg -n "signed-url-revoked|revoke|revoked|revocation|강제 무효화|TTL 만료 전" docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md docs/admin/REVIEW_WORKFLOW.md docs/decisions/INFRA_DECISIONS_DRAFT.md' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'rg -n "fallback|reversal blast radius|INFRA2-01|INFRA3-03|Auth.js Drizzle adapter|Drizzle Kit RLS|외부 call duplicate|D FAIL|E FAIL" docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command 'rg -n "PASS with unresolved|FAIL|C-local PASS|C-provider PASS|A-provider PASS|D PASS|E PASS|Day 10" docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md' in C:\Users\assag\solution\website-exposure
 succeeded in 669ms:
docs/core/DATA_MODEL.md:968:| `role` | `AdminUserRole` (단 `system` 제외) | ✅ | `admin/REVIEW_WORKFLOW.md` § 11.1 enum 6종 중 실제 사용자 역할 5종(`super-admin`·`operator`·`physician-reviewer`·`legal-reviewer`·`client-approver`). **`system`은 audit log actorRole 표기 전용** — AdminUser DB row 미생성, 로그인 불가. C-23.`role` 및 `instanceMemberships[].role`에는 저장 금지 |
docs/core/DATA_MODEL.md:974:| `instanceMemberships` | `Array<{instanceId: Slug, role: AdminUserRole, joinedAt: Date}>` | ✅ | 사용자가 접근 가능한 인스턴스 목록 (multi-tenant) |
...
docs/core/DATA_MODEL.md:1107:| 2026-05-14 | v0.13 | **`features/notifications.md` cascade (1차+3차 사이클 통합)**: (1) **C-08 확장** — `adminBaseUrl`(URL, notifications 활성 시 required) + `timezone`(IANATimezone, notifications·SLA 활성 시 required) + `notificationChannels`를 `NotificationChannelsConfig`로 확장(email transport·secretRef·sender·rateLimit / slack webhookUrlSecretRef·rateLimit / inApp) + **`holidayCalendar`(region·source — 3차 cycle N3-13)**, (2) **C-23 `AdminUser` 신설** — 어드민 사용자·자격·알림 선호 SoT. `id`·`email`·`role`(AdminUserRole)·`approverRoleEligibility[]`·`eligibilityEvidence[]`·`slackUserId`·`timezone`(quietHours 한정 — 3차 cycle N3-20)·`notificationPreferences`(channels·digestOptOut·quietHours·**suppression with autoReleaseAt** — 3차 cycle N3-15)·`instanceMemberships[]`·`active`, (3) **`IANATimezone` 공통 타입 표기** (IANA Time Zone Database 식별자), (4) 인벤토리 22개 → 23개 |
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:316:> Auth.js (next-auth) magic link로 로그인·session 생성·next request에서 `resolveTenantContext(session, requestedInstanceId)` 호출 → instanceMembership·active·role·legal eligibility 검증 → `app.current_instance_id` set. client-supplied `requestedInstanceId` tampering 차단. membership 제거·deactivation 후 next request 즉시 403. super-admin instance switch audit.
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:324:│   ├── 012_admin_user.sql           # AdminUser + instanceMemberships
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:333:2. instanceMembership: A는 instance-a operator
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:334:3. `requestedInstanceId='instance-a'` 요청 → 통과·withTenantTransaction 진입
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:335:4. `requestedInstanceId='instance-b'` 요청 (A는 instance-b 미멤버) → 403
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:337:6. **membership 제거 후 next request**: A의 instance-a membership active=false → 다음 요청 403 (session expiry 안 기다림)
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:338:7. **inactive user**: A.active=false → 모든 요청 403
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:341:10. **session refresh**: 매 요청 server-side 검증 (session expiry 안 기다림)
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:350:| membership 제거 후 next request | 즉시 403 (session refresh 없이) |
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:360:| session refresh latency 큼 | session cache + invalidation token | Auth.js 추가 정책 |
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:361:| instanceMembership 검증 query 비용 큼 | scoped index·또는 short-cache | acceptable trade-off |
docs/decisions/INFRA_DECISIONS_DRAFT.md:113:  requestedInstanceId: string
docs/decisions/INFRA_DECISIONS_DRAFT.md:115:  // 1. instanceMemberships에 requestedInstanceId 포함 여부 검증
docs/decisions/INFRA_DECISIONS_DRAFT.md:117:    .findInstanceMembership(session.userId, requestedInstanceId);
docs/decisions/INFRA_DECISIONS_DRAFT.md:121:  if (!membership.active) throw new HttpError(403, 'inactive-member');
docs/decisions/INFRA_DECISIONS_DRAFT.md:129:  if (session.previousInstanceId && session.previousInstanceId !== requestedInstanceId) {
docs/decisions/INFRA_DECISIONS_DRAFT.md:133:      metadata: { from: session.previousInstanceId, to: requestedInstanceId }
docs/decisions/INFRA_DECISIONS_DRAFT.md:137:  return { instanceId: requestedInstanceId, role: membership.role, ... };
docs/decisions/INFRA_DECISIONS_DRAFT.md:143:- client가 보낸 `requestedInstanceId`를 검증 없이 신뢰 금지

 succeeded in 629ms:
26:| Day 10 (Wed Week 2) | 종합 보고서·v1.0 결정 갱신·Week 3~6 계획 분기 | dependency graph 결과 반영 |
47:provider smoke 통과 전에는 **PASS with unresolved provider risk** 까지만 인정. v1.0 acceptance는 provider smoke 후.
129:- A FAIL → Phase 0 scope 재산정 gate (Week 2 Day 10에 의사결정)
190:- B FAIL → worker 패턴 재설계·notifications P0 subset 재정의
243:| minio와 R2 동작 차이 큼 | C-provider만 권위·minio는 logic만 | C-local PASS 가치 약화·provider smoke 통과 필수 |
247:- C-local PASS → signing logic·tenant-check
248:- C-provider PASS → `packages/storage`·blobRef schema·upload/read API·asset-ingestion·search-visibility·content-migration storage 적용
249:- C FAIL → Storage ADR 재작성·Phase 1+ 영향
307:- D PASS → Week 3~4 M0 vertical slice schema migration 진행
308:- D FAIL → migration tooling 재선택 (Atlas·db-mate·dbmate 등)·Phase 0 scope 재정의
365:- E PASS → `packages/auth`·`resolveTenantContext`·admin UI middleware·session 패턴
366:- E FAIL → Auth provider 변경·Phase 0 Week 2~3 일정 영향
389:Day 5    C-local PASS ──┐
390:Day 8    C-provider PASS ──┴─→ Week 5+ packages/storage·blobRef·feature blob 적용
392:Day 6    D PASS ──┴─→ Week 4 M0 vertical slice migration 진행
394:Day 7    E PASS ──┴─→ Week 6 admin UI middleware·session·login flow
396:Day 9    A-provider PASS (Supabase Pooler) ──→ A 결정 v1.0 확정
397:Day 10   종합 보고서·dependency 확인·Week 3~6 계획 분기
401:- A FAIL → 전체 Phase 0 scope 재산정 (Week 2 Day 10 gate)
402:- B FAIL → worker 패턴 재설계·notifications P0 subset 재정의
403:- C FAIL → Storage ADR 변경·Phase 1+ 영향
404:- D FAIL → migration tooling 재선택
405:- E FAIL → Auth provider 변경

 succeeded in 646ms:
33:- **가설 (hypothesis)**·**실험 (experiment)**·**pass criteria**·**negative invariant 측정 방법**·**provider smoke 분리**·**실패 시 fallback + reversal blast radius**·**downstream unblock**
117:### A.4 실패 시 대안 + reversal blast radius (SPIKE1-14)
119:| 실패 항목 | fallback | reversal blast radius |
121:| pgBouncer SET LOCAL leak | Supabase Pooler 사용 또는 direct connection·또는 connection-scoped role | 비용·확장성 trade-off. INFRA2-01 일부 변경 |
123:| RLS overhead p95 > 50ms | application-level scoping 주력·RLS 보조만 | INFRA2-01 전체 reversal — INFRA 결정 v1.0 §1.1 재작성·1차 결정으로 reversal |
171:| 100건 처리 (대량 100건) | duplicate inbox 0건·외부 call duplicate 0건·foreign instance 0건 |
178:### B.4 fallback + reversal
180:| 실패 | fallback | reversal blast radius |
185:| 외부 call duplicate | provider별 idempotency-key header 강제 | 8 feature 외부 호출 패턴 재검토 |
237:### C.5 fallback + reversal
239:| 실패 | fallback | reversal blast radius |
241:| R2 signed URL 동작 안 함 | Cloudflare Workers + R2 binding | INFRA3-03 Storage ADR 변경·일부 reversal |
242:| R2 IAM 미흡 | Supabase Storage로 reversal | INFRA3-03 reversal·3 feature blob storage 계약 재작성 |
297:### D.4 fallback + reversal
299:| 실패 | fallback | reversal blast radius |
301:| Drizzle Kit RLS policy 생성 불가 | raw SQL migration mixin | migration tooling 약간 복잡·관리 비용 |
308:- D FAIL → migration tooling 재선택 (Atlas·db-mate·dbmate 등)·Phase 0 scope 재정의
355:### E.4 fallback + reversal
357:| 실패 | fallback | reversal blast radius |
359:| Auth.js Drizzle adapter 호환 안 됨 | Lucia·Better-Auth로 전환 | INFRA 결정 v1.0 §2 stack reversal |
366:- E FAIL → Auth provider 변경·Phase 0 Week 2~3 일정 영향
404:- D FAIL → migration tooling 재선택
405:- E FAIL → Auth provider 변경
414:| 2026-05-15 | **v0.2** | **codex 1차 비평 18 지적 전건 수용**: (1) **provider smoke gate** — local + provider 2단계 분리 (SPIKE1-01·04), (2) **Spike D 신설** — Drizzle Kit migration deploy (SPIKE1-02), (3) **Spike E 신설** — Auth.js + resolveTenantContext + membership invalidation (SPIKE1-03), (4) **invariant 측정 표준화** — 1000 iterations × 20 concurrent + invariant SQL binary 검증 (SPIKE1-05), (5) **A 쓰기 path·rollback·nested tx·audit_log RLS 시나리오 추가** (SPIKE1-06·07), (6) **B 가설 정정** — idempotent at-least-once with exactly-once observable effects (SPIKE1-08), (7) **B failure injection 8 point + 외부 call count invariant** (SPIKE1-09·10), (8) **C TTL 상태 코드 정정** — 401 → 403 (provider semantics·SPIKE1-11), (9) **C security 시나리오 확장** — replay·method confusion·ListBucket·content-type·URL scrubbing (SPIKE1-12), (10) **REVIEW_WORKFLOW cascade** — `signed-url-issued`·`signed-url-revoked` AuditAction 2종 추가 (SPIKE1-13), (11) **fallback에 reversal blast radius 명시** (SPIKE1-14), (12) **Week 3~6 dependency graph** (SPIKE1-15), (13) **일정 7일 → 10일** (Week 1 + Week 2 Day 8~10·SPIKE1-16), (14) **성능 기준 correctness와 분리** (SPIKE1-17), (15) **A/B scope 명확화** — A는 transaction/RLS·B는 queue/crash (SPIKE1-18), (16) Spike F·G P1·후속 명시 |
 ...
 succeeded in 667ms:
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:53:- `signed-url-revoked` — TTL 만료 전 강제 무효화
docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md:414:| 2026-05-15 | **v0.2** | **codex 1차 비평 18 지적 전건 수용**: (1) **provider smoke gate** — local + provider 2단계 분리 (SPIKE1-01·04), (2) **Spike D 신설** — Drizzle Kit migration deploy (SPIKE1-02), (3) **Spike E 신설** — Auth.js + resolveTenantContext + membership invalidation (SPIKE1-03), (4) **invariant 측정 표준화** — 1000 iterations × 20 concurrent + invariant SQL binary 검증 (SPIKE1-05), (5) **A 쓰기 path·rollback·nested tx·audit_log RLS 시나리오 추가** (SPIKE1-06·07), (6) **B 가설 정정** — idempotent at-least-once with exactly-once observable effects (SPIKE1-08), (7) **B failure injection 8 point + 외부 call count invariant** (SPIKE1-09·10), (8) **C TTL 상태 코드 정정** — 401 → 403 (provider semantics·SPIKE1-11), (9) **C security 시나리오 확장** — replay·method confusion·ListBucket·content-type·URL scrubbing (SPIKE1-12), (10) **REVIEW_WORKFLOW cascade** — `signed-url-issued`·`signed-url-revoked` AuditAction 2종 추가 (SPIKE1-13), (11) **fallback에 reversal blast radius 명시** (SPIKE1-14), (12) **Week 3~6 dependency graph** (SPIKE1-15), (13) **일정 7일 → 10일** (Week 1 + Week 2 Day 8~10·SPIKE1-16), (14) **성능 기준 correctness와 분리** (SPIKE1-17), (15) **A/B scope 명확화** — A는 transaction/RLS·B는 queue/crash (SPIKE1-18), (16) Spike F·G P1·후속 명시 |
docs/admin/REVIEW_WORKFLOW.md:712:  | "signed-url-revoked";                     // SPIKE1-13 — signed URL TTL 만료 전 강제 무효화 (운영 incident 대응)
...
codex
{
  ...
  "ready_for_acceptance": false,
  "summary": {
    "verdict": "v0.2는 1차 18개 지적의 대부분을 문서상 수용했지만, v1.0 acceptance에는 아직 이릅니다. 핵심 차단 사유는 SoT cascade 불일치와 Auth.js/Spike E provider smoke gate 누락입니다.",
    "blocking": 2,
    "major": 4,
    "minor": 1
  },
  "recurrence_check": {
    "SPIKE1-01": "partial_recurrence: provider smoke gate는 추가됐지만 Spike E/Auth.js provider smoke 일정과 pass criteria가 없음",
    "SPIKE1-02": "resolved: Spike D 신설",
    "SPIKE1-03": "partial_recurrence: Spike E 신설됐으나 실제 Vercel/Auth.js callback/cookie smoke 없음",
    "SPIKE1-04": "resolved: C-provider R2 smoke 추가",
    "SPIKE1-05": "resolved: invariant 반복 기준 추가",
    "SPIKE1-06": "resolved: A write path 추가",
    "SPIKE1-07": "resolved: audit_log RLS/append-only 추가",
    "SPIKE1-08": "resolved: B 가설 정정",
    "SPIKE1-09": "resolved: B failure injection 8 point 추가",
    "SPIKE1-10": "resolved: fake provider call count 추가",
    "SPIKE1-11": "resolved: provider 실제 TTL status 기록으로 정정",
    "SPIKE1-12": "resolved: C security negative tests 추가",
    "SPIKE1-13": "resolved: REVIEW_WORKFLOW AuditAction cascade 확인됨",
    "SPIKE1-14": "partial_recurrence: reversal blast radius는 추가됐지만 실제 affected SoT/package/schedule cascade는 아직 얕음",
    "SPIKE1-15": "partial_recurrence: dependency graph는 추가됐지만 partial/provider-fail 분기 부족",
    "SPIKE1-16": "mostly_resolved: 10일로 확장했으나 D/E 1일 cutline은 여전히 빡빡함",
    "SPIKE1-17": "resolved: correctness와 performance 분리",
    "SPIKE1-18": "resolved: A/B scope 분리"
  },
  "findings": [
    {
      "id": "SPIKE2-01",
      "severity": "blocking",
      "category": "sot-drift",
      "location": "PHASE0_WEEK1_SPIKES_DRAFT.md:17-28, INFRA_DECISIONS_DRAFT.md:296-313",
      "issue": "Spike 문서는 5개 Spike를 Week 1~2 10일 gate로 확장했지만, 인프라 SoT는 아직 Week 1 A/B/C 3개 Spike와 Week 2 본 작업으로 남아 있습니다.",
      "impact": "v1.0 acceptance 기준의 SoT cascade 동기화가 깨집니다. 어떤 문서를 기준으로 Week 2 Supabase/Drizzle/Auth.js 본 작업을 시작할지 모호하고, provider smoke가 본 작업 선행 gate인지 병행 작업인지 불명확합니다.",
      "recommendation": "INFRA_DECISIONS_DRAFT §4.1~4.2를 5 Spike/10일 계획으로 cascade하거나, Spike 문서에 '인프라 v1.0의 Week 1 gate를 대체하는 후속 결정'임을 명시하고 acceptance 전 SoT 변경 이력을 반영하세요."
    },
    {
      "id": "SPIKE2-02",
      "severity": "blocking",
      "category": "provider-smoke-gap",
      "location": "PHASE0_WEEK1_SPIKES_DRAFT.md:41-47, 24-25, 312-351",
      "issue": "공통 규약은 각 Spike가 local + provider 2단계라고 하지만, 실제 일정의 provider smoke는 C-provider와 A-provider뿐입니다. Spike E는 Auth.js production-like/Vercel preview smoke가 없습니다.",
      "impact": "1차 SPIKE1-01·03의 핵심 재발입니다. Auth.js magic link, callback URL, secure cookie, SameSite, Drizzle adapter session persistence, Vercel preview 환경의 request/session boundary는 로컬 mock으로 검증되지 않습니다.",
      "recommendation": "E-provider smoke를 Day 8~10 gate에 추가하세요. 최소 Vercel preview 또는 production-like runtime에서 magic link callback 1회, session 생성/복원, requestedInstanceId tampering 403, membership active=false 후 next request 403, Supabase dev DB Drizzle adapter persistence를 pass criteria로 고정해야 합니다."
    },
    {
      "id": "SPIKE2-03",
      "severity": "major",
      "category": "data-model-drift",
      "location": "PHASE0_WEEK1_SPIKES_DRAFT.md:316-350, INFRA_DECISIONS_DRAFT.md:115-121, DATA_MODEL.md:968-975",
      "issue": "Spike E와 INFRA의 resolveTenantContext는 membership.active를 전제로 하지만 DATA_MODEL C-23의 instanceMemberships에는 active 필드가 없습니다.",
      "impact": "membership 제거 후 next request 즉시 403이라는 Spike E acceptance를 실제 SoT schema로 구현할 수 없습니다. 별도 join table을 만들지, C-23 embedded membership에 active/disabledAt을 추가할지 결정되지 않았습니다.",
      "recommendation": "DATA_MODEL C-23에 instanceMemberships[].active, deactivatedAt, deactivatedBy를 추가하거나, AdminUserInstanceMembership 별도 DB table을 SoT로 분리하세요. Spike E migration 012_admin_user.sql도 그 결정과 일치해야 합니다."
    },
    {
      "id": "SPIKE2-04",
      "severity": "major",
      "category": "storage-revocation-gap",
      "location": "PHASE0_WEEK1_SPIKES_DRAFT.md:52-53, REVIEW_WORKFLOW.md:711-712",
      "issue": "signed-url-revoked AuditAction은 추가됐지만, TTL 만료 전 강제 무효화가 실제로 어떤 메커니즘으로 가능한지 Spike C에 검증 시나리오가 없습니다.",
      "impact": "R2/S3 presigned URL은 일반적으로 bearer URL이라 직접 발급한 URL을 TTL 전에 선별 revoke하기 어렵습니다. action enum만 추가하면 운영자가 가능한 기능으로 오해할 수 있습니다.",
      "recommendation": "C-provider에 revoke 메커니즘을 명시하세요. 예: object key rotation/delete, credential rotation, short TTL only, Cloudflare Worker proxy denylist 중 하나를 선택하고, revoked 후 기존 URL이 object body를 반환하지 않는지 검증하세요. 불가능하면 AuditAction 의미를 'revocation-requested' 또는 'object-access-disabled'로 바꾸세요."
    },
    {
      "id": "SPIKE2-05",
      "severity": "major",
      "category": "dependency-graph",
      "location": "PHASE0_WEEK1_SPIKES_DRAFT.md:382-405",
      "issue": "dependency graph가 PASS와 FAIL만 다루고, PASS with unresolved provider risk, local PASS/provider FAIL, D/E 지연, E-provider 누락 같은 중간 상태를 다루지 않습니다.",
      "impact": "Day 10 의사결정에서 실제로 가장 가능성 높은 상태인 'A local PASS but provider pending', 'C-local PASS but R2 IAM FAIL', 'E local PASS but preview 미검증'에 대한 Week 3~6 분기가 없습니다.",
      "recommendation": "각 Spike별 상태를 LOCAL_PASS, PROVIDER_PASS, PROVIDER_FAIL, INCONCLUSIVE로 나누고 Week 3~6 unlock/hold 규칙을 표로 추가하세요. 특히 E_PROVIDER_PASS 전에는 admin UI middleware/login flow 착수를 hold해야 합니다."
    },
    {
      "id": "SPIKE2-06",
      "severity": "major",
      "category": "reversal-blast-radius",
      "location": "PHASE0_WEEK1_SPIKES_DRAFT.md:117-123, 178-185, 237-242, 297-301, 355-361",
      "issue": "fallback별 reversal blast radius가 추가됐지만 실제 SoT cascade 영향 평가로는 부족합니다. affected docs, packages, migrations, schedule delta가 빠져 있습니다.",
      "impact": "A/C/D/E 실패 시 어떤 v1.0 문서와 Week 3~6 작업을 멈추거나 재작성해야 하는지 Day 10에서 바로 결정하기 어렵습니다.",
      "recommendation": "각 fallback 행에 affected SoT docs, affected package/module, data migration impact, schedule delta, owner를 추가하세요. 예: E Auth provider 변경은 INFRA §1.3/§2 stack, DATA_MODEL C-23 session/membership, REVIEW_WORKFLOW login/logout audit, packages/auth, admin middleware 전체에 영향이 있습니다."
    },
    {
      "id": "SPIKE2-07",
      "severity": "minor",
      "category": "schedule-realism",
      "location": "PHASE0_WEEK1_SPIKES_DRAFT.md:17-28, 253-366",
      "issue": "10일 확장은 7일안보다 현실적이지만, D와 E를 각각 하루에 끝내는 계획은 pass/fail artifact 기준이 없으면 솔로 운영에서 흔들립니다.",
      "impact": "D/E가 '프로토타입 작성'으로 끝나고 migration report, preview smoke evidence, failure log 같은 acceptance artifact가 누락될 수 있습니다.",
      "recommendation": "D/E에는 하루 안에 반드시 남길 산출물을 제한적으로 정의하세요. 예: D는 generated SQL + staging apply log + drift failure sample, E는 provider smoke report + 403 matrix + session DB row 증거."
    }
  ],
  "new_critical_scenarios_missing": [
    {
      "id": "SPIKE2-CS-01",
      "title": "Auth.js provider smoke in Vercel preview",
      "why": "로컬 Auth.js mock은 cookie/callback/session persistence 리스크를 대체하지 못합니다.",
      "suggested_spike": "Spike E-provider"
    },
    {
      "id": "SPIKE2-CS-02",
      "title": "Signed URL early revocation feasibility",
      "why": "signed-url-revoked enum이 실제 보안 기능을 암시하지만 C-provider가 검증하지 않습니다.",
      "suggested_spike": "Spike C-provider"
    },
    {
      "id": "SPIKE2-CS-03",
      "title": "Membership schema SoT cascade",
      "why": "membership.active 검증은 핵심 권한 invariant인데 DATA_MODEL C-23에 필드가 없습니다.",
      "suggested_spike": "Spike E / DATA_MODEL cascade"
    },
    {
      "id": "SPIKE2-CS-04",
      "title": "Partial-pass Day 10 decision matrix",
      "why": "provider FAIL 또는 unresolved provider risk 상태에서 Week 3~6을 어떻게 hold/resume할지 결정 규칙이 없습니다.",
      "suggested_spike": "종합 dependency graph"
    }
  ],
  "acceptance_conditions": [
    "INFRA_DECISIONS_DRAFT §4.1~4.2가 5 Spike/10일 provider gate 계획과 동기화되어야 한다.",
    "Spike E-provider smoke gate를 추가하고 v1.0 acceptance는 E-provider PASS 후로 제한해야 한다.",
    "DATA_MODEL C-23 membership active/deactivation 모델을 Spike E와 resolveTenantContext에 맞게 cascade해야 한다.",
    "signed-url-revoked의 실제 revoke 메커니즘 또는 enum 의미를 재정의해야 한다.",
    "dependency graph와 reversal blast radius가 partial/provider-fail 분기 및 affected SoT/package/schedule delta를 포함해야 한다."
  ]
}
tokens used
77,974
...
codex
핵심 cascade는 대부분 반영된 것으로 보입니다. 다만 acceptance 판정 전에 문서 내부 잔재 표현과 SoT 요약부 불일치가 실제 운영 판단을 흔들 정도인지 라인 단위로 확인하겠습니다.
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command "Select-String -Path docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md -Pattern 'provider smoke|Day 10|E-provider|signed-url|active|Week 3-6|A·D PROVIDER_PASS|D "'`LOCAL_PASS`|D `INCONCLUSIVE`|C `PROVIDER_PASS`'"' | ForEach-Object { \""'$($_.LineNumber):$($_.Line)" }' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command "Select-String -Path docs/decisions/INFRA_DECISIONS_DRAFT.md -Pattern '4\\.1|4\\.2|Week 1|A·B·C|5개 Spike|Day 10|E "'`PROVIDER_PASS`|E `LOCAL_PASS|종합 결정|Phase 0'"' | ForEach-Object { \""'$($_.LineNumber):$($_.Line)" }' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command "Select-String -Path docs/admin/REVIEW_WORKFLOW.md -Pattern 'signed-url-issued|signed-url-revocation-requested|signed-url-revoked|service-role-invoked|instance-switched' | ForEach-Object { \""'$($_.LineNumber):$($_.Line)" }' in C:\Users\assag\solution\website-exposure
exec
"C:\\Users\\assag\\AppData\\Local\\Microsoft\\WindowsApps\\pwsh.exe" -Command "Select-String -Path docs/core/DATA_MODEL.md -Pattern 'v0\\.24|instanceMemberships|active=false|deactivatedAt|deactivatedBy|resolveTenantContext' | ForEach-Object { \""'$($_.LineNumber):$($_.Line)" }' in C:\Users\assag\solution\website-exposure
 succeeded in 755ms:
708:  | "service-role-invoked"                    // INFRA2-02 — service_role break-glass 사용 추적
709:  | "instance-switched"                       // INFRA2-08 — super-admin cross-instance 전환
711:  | "signed-url-issued"                       // R2 signed URL 발급 (signature 미저장·prefix·objectKey만)
712:  | "signed-url-revocation-requested";        // SPIKE2-04 정정 — 즉시 revoke 불가능 (R2/S3 presigned URL은 bearer). 운영자 revoke 요청 → credential rotation 또는 object key rotation으로 후속 처리
...
24:| Day 8 (Mon Week 2) | C-provider — R2 staging IAM·credential·real presigned URL | provider smoke gate |
25:| Day 9 (Tue Week 2) | A-provider — Supabase Pooler transaction mode + RLS smoke | provider smoke gate |
26:| Day 10 (Wed Week 2) | **E-provider** — Vercel preview·Auth.js magic link callback·session persistence·tampering·403 matrix (SPIKE2-02) + 종합 보고서·v1.0 결정 갱신·Week 3~6 계획 분기 | provider smoke gate + dependency graph |
33:- **가설 (hypothesis)**·**실험 (experiment)**·**pass criteria**·**negative invariant 측정 방법**·**provider smoke 분리**·**실패 시 fallback + reversal blast radius**·**downstream unblock**
41:### 0.3 provider smoke gate (SPIKE1-01·04 — 핵심 정정)
47:provider smoke 통과 전에는 **PASS with unresolved provider risk** 까지만 인정. v1.0 acceptance는 provider smoke 후.
52:- `signed-url-issued` — R2 signed URL 발급 추적
53:- `signed-url-revocation-requested` (SPIKE2-04 정정 — rename from `signed-url-revoked`) — 즉시 revoke 불가능 (R2/S3 presigned URL은 bearer URL). 운영자 revoke 요청 → 후속 credential rotation 또는 object key rotation으로 처리
57:`AdminUser.instanceMemberships`에 `active`·`deactivatedAt`·`deactivatedBy` 필드 추가 (v0.24). resolveTenantContext가 매 요청 검증.
133:- A FAIL → Phase 0 scope 재산정 gate (Week 2 Day 10에 의사결정)
154:│   ├── fake-provider.ts             # idempotency key 별 call count 기록
166:6. **외부 call count invariant**: fake-provider.callCount(sourceEventId) = 1 (1000 jobs × 10 workers × 20 runs)
232:### C.4-provider: R2 staging IAM·real presigned (Day 8 — provider smoke gate)
239:| cross-instance copy (import) | service_role만 가능 + `signed-url-issued` audit |
247:| minio와 R2 동작 차이 큼 | C-provider만 권위·minio는 logic만 | C-local PASS 가치 약화·provider smoke 통과 필수 |
270:│   ├── 008_partial_unique.sql       # WHERE active=true unique
328:> Auth.js (next-auth) magic link로 로그인·session 생성·next request에서 `resolveTenantContext(session, requestedInstanceId)` 호출 → instanceMembership·active·role·legal eligibility 검증 → `app.current_instance_id` set. client-supplied `requestedInstanceId` tampering 차단. membership 제거·deactivation 후 next request 즉시 403. super-admin instance switch audit.
349:6. **membership 제거 후 next request**: A의 instance-a membership active=false → 다음 요청 403 (session expiry 안 기다림)
350:7. **inactive user**: A.active=false → 모든 요청 403
355:### E.2-provider: Vercel preview + Auth.js production-like (Day 10 — SPIKE2-02 신규)
363:| membership active=false 후 next request | 즉시 403 (session expiry 안 기다림) |
374:| inactive user | 100% 403 |
418:| Spike 상태 | Week 3-6 unlock | Week 3-6 hold |
425:| C `PROVIDER_PASS` | Week 5+ packages/storage·blobRef·feature blob 적용·asset-ingestion·search-visibility·content-migration storage |
427:| D `LOCAL_PASS` | Week 4 M0 vertical slice migration 진행 |
428:| D `INCONCLUSIVE` | — | migration tooling 재선택 (Atlas·db-mate)·Phase 0 scope 재정의 |
433:### 의사결정 매트릭스 (Day 10 종합 보고서)
437:| A·D PROVIDER_PASS + B·C·E LOCAL_PASS | Week 3 schema/migration·worker skeleton·UI 컴포넌트 | Week 4 production-readiness·Storage 적용·admin login flow until provider gate | minor |
449:| 2026-05-15 | **v0.3** | **codex 2차 비평 7 지적 전건 수용 + SoT cascade**: (1) INFRA v1.0 §4.1·§4.2 5 Spike/10일 동기화 (SPIKE2-01), (2) **E-provider smoke gate Day 10 추가** — Vercel preview + Auth.js production-like + session DB persistence + 403 matrix (SPIKE2-02), (3) **DATA_MODEL C-23 v0.24 cascade** — instanceMemberships에 active·deactivatedAt·deactivatedBy (SPIKE2-03), (4) **REVIEW_WORKFLOW signed-url-revoked → signed-url-revocation-requested rename** — bearer URL은 즉시 revoke 불가·credential/object rotation으로 후속 처리 (SPIKE2-04), (5) **partial state matrix + Week 3-6 unlock/hold 규칙** — LOCAL_PASS·PROVIDER_PASS·PROVIDER_FAIL·INCONCLUSIVE (SPIKE2-05), (6) **A fallback reversal blast radius 상세** — affected SoT·packages·schedule delta·owner (SPIKE2-06), (7) **D·E artifact 명시** — D는 SQL/log/drift sample·E는 preview report/403 matrix/session DB row (SPIKE2-07) |
450:| 2026-05-15 | (v0.2 이전 비고) | codex 1차 비평 18 지적 전건 수용: (1) **provider smoke gate** — local + provider 2단계 분리 (SPIKE1-01·04), (2) **Spike D 신설** — Drizzle Kit migration deploy (SPIKE1-02), (3) **Spike E 신설** — Auth.js + resolveTenantContext + membership invalidation (SPIKE1-03), (4) **invariant 측정 표준화** — 1000 iterations × 20 concurrent + invariant SQL binary 검증 (SPIKE1-05), (5) **A 쓰기 path·rollback·nested tx·audit_log RLS 시나리오 추가** (SPIKE1-06·07), (6) **B 가설 정정** — idempotent at-least-once with exactly-once observable effects (SPIKE1-08), (7) **B failure injection 8 point + 외부 call count invariant** (SPIKE1-09·10), (8) **C TTL 상태 코드 정정** — 401 → 403 (provider semantics·SPIKE1-11), (9) **C security 시나리오 확장** — replay·method confusion·ListBucket·content-type·URL scrubbing (SPIKE1-12), (10) **REVIEW_WORKFLOW cascade** — `signed-url-issued`·`signed-url-revoked` AuditAction 2종 추가 (SPIKE1-13), (11) **fallback에 reversal blast radius 명시** (SPIKE1-14), (12) **Week 3~6 dependency graph** (SPIKE1-15), (13) **일정 7일 → 10일** (Week 1 + Week 2 Day 8~10·SPIKE1-16), (14) **성능 기준 correctness와 분리** (SPIKE1-17), (15) **A/B scope 명확화** — A는 transaction/RLS·B는 queue/crash (SPIKE1-18), (16) Spike F·G P1·후속 명시 |
...
1108:| 2026-05-14 | v0.13 | **`features/notifications.md` cascade (1차+3차 사이클 통합)**: (1) **C-08 확장** — `adminBaseUrl`(URL, notifications 활성 시 required) + `timezone`(IANATimezone, notifications·SLA 활성 시 required) + `notificationChannels`를 `NotificationChannelsConfig`로 확장(email transport·secretRef·sender·rateLimit / slack webhookUrlSecretRef·rateLimit / inApp) + **`holidayCalendar`(region·source — 3차 cycle N3-13)**, (2) **C-23 `AdminUser` 신설** — 어드민 사용자·자격·알림 선호 SoT. `id`·`email`·`role`(AdminUserRole)·`approverRoleEligibility[]`·`eligibilityEvidence[]`·`slackUserId`·`timezone`(quietHours 한정 — 3차 cycle N3-20)·`notificationPreferences`(channels·digestOptOut·quietHours·**suppression with autoReleaseAt** — 3차 cycle N3-15)·`instanceMemberships[]`·`active`, (3) **`IANATimezone` 공통 타입 표기** (IANA Time Zone Database 식별자), (4) 인벤토리 22개 → 23개 |
 ...
 succeeded in 788ms:
7:> **상위 결정**: 8 Feature spec v1.0 완료·솔로 1명 + AI 보조·9~12개월·Phase 0~4 단계·Vercel+Supabase+Railway 스택·next-auth
8:> **핵심 변경 (v0.3)**: RLS 실행 모델·service-role audit cascade·Phase 0 outbox 분류·tenant export manifest dependency class·Storage ADR 옵션·resolveTenantContext·Phase 0 spike gate·legal-reviewer contract·internal beta 범위 제한·customer domain ADR·사전심의 manual-assisted·PIPA+GDPR checklist·email transport/provider 분리
214:Phase 0~1에 control-plane vs tenant-plane 분류 ADR 별도 작성. 본 문서는 분류 진입점만 명시.
294:## 영역 4: Phase 0 작업 + 베타·법무 일정
296:### 4.1 Phase 0 Spike Gate (INFRA2-09 → SPIKE2-01 cascade — 5 Spike/10일로 확장)
298:Week 1~2 Day 1~10에 5개 Spike (A·B·C·D·E)를 별도 gate로 진행. 상세 계획·시나리오·invariant 측정은 **`docs/decisions/PHASE0_WEEK1_SPIKES_DRAFT.md`** SoT.
306:| **E. Auth.js + resolveTenantContext + membership** | magic link·session·instanceMembership 검증·tampering 차단·active=false 즉시 403 | Day 7 local·Day 10 provider (Vercel preview·SPIKE2-02) | local + provider |
308:Spike 통과 후 Week 3~6 본 작업 진행. Day 10 종합 보고서·v1.0 갱신·Week 3~6 분기.
310:### 4.2 Spike 결과별 Week 3~6 dependency (SPIKE2-05 partial state matrix)
321:- E `PROVIDER_PASS` → Week 6 admin UI middleware·login flow 착수
322:- E `LOCAL_PASS but PROVIDER_PASS pending` → admin UI middleware **hold** until provider gate
326:### 4.2 Phase 0 작업 (v0.2 § 3 + spike 추가)
330:| **Week 1 (Spike)** | Spike A·B·C 진행 + repo skeleton + tsconfig + Biome + CI 기본 | Spike 통과 |
336:| Week 6 | 운영자 UI 골격·M0 e2e 1개 (login → 콘텐츠 작성 → audit log row 확인) | Phase 0 sign-off |
338:### 4.3 Phase 0 outbox 분류 — **옵션 A 선택** (INFRA2-03)
373:| Phase 0 (Week 1~6) | **Fixed-scope package** — DPA 템플릿 1회·의료광고 워크플로 2회 워크숍·high-risk sample 20건 검토·사전심의 절차 자문 1회 | 500~1500만원 (1회성) |
436:## 종합 결정 요약 (v0.3)
445:| 4-1. Phase 0 | 6~8주·Week 1 Spike A·B·C gate·M0 vertical slice schema·P0 outbox subset (notifications 4 tables 포함) |
471:| 2026-05-15 | (v0.3 비고 이전) | **codex 2차 15 지적 전건 수용 + cascade**: (1) **RLS 실행 모델** — withTenantTransaction 헬퍼·SET LOCAL·worker control/tenant plane 분리·pgBouncer transaction pooling·lint·runtime guard (INFRA2-01), (2) **REVIEW_WORKFLOW cascade — service-role-invoked·instance-switched AuditAction 2종 추가** (INFRA2-02·08), (3) **Phase 0 outbox 옵션 A** — P0에 notifications 최소 subset (Receipt·Log·PayloadRecord·DeliveryAttempt) 포함 (INFRA2-03), (4) **composite FK 3등급 분류** — tenant-plane hard FK·control-plane FK·polymorphic ref typed registry (INFRA2-04), (5) **tenant export/import manifest dependency class** — portable·rebind-required·rotate-required·legal-reapproval-required·external-provider-owned·blob-copy-required·audit-chain-preserved (INFRA2-05), (6) **rate limit taxonomy** — Postgres hard quota·Redis soft cache 분리 (INFRA2-06), (7) **Storage ADR — Cloudflare R2 reversal 권장** (INFRA2-07), (8) **resolveTenantContext** — server-side membership/role/legal eligibility 검증·instance-switched audit (INFRA2-08), (9) **Spike A·B·C gate Week 1** (INFRA2-09), (10) **legal-reviewer fixed-scope package → 시간당 → retainer 단계** (INFRA2-10), (11) **internal beta는 workflow technical validation 한정** (INFRA2-11), (12) **customer domain ADR 별도** (INFRA2-12), (13) **사전심의 manual-assisted workflow** — submission packet export·institutionType enum (INFRA2-13), (14) **PIPA + GDPR checklist** Phase 1 gate (INFRA2-14), (15) **DATA_MODEL C-08 v0.23 cascade — email transport/provider 분리** (INFRA2-15) |
...
codex
{
  ...
  "summary": {
    "verdict": "v0.3는 v1.0 acceptance 가능입니다. 2차 blocking 2건은 해소됐고, REVIEW_WORKFLOW·DATA_MODEL·INFRA의 핵심 cascade도 반영됐습니다. 남은 지적은 Day 10 의사결정 표의 용어 오류와 INFRA 요약부 잔재 수준이며, 솔로 + AI 보조 운영을 막지는 않습니다.",
    "blocking": 0,
    "major": 1,
    "minor": 1
  },
  "recurrence_check": {
    "SPIKE2-01": "resolved: INFRA §4.1이 5 Spike/10일 gate와 Day 8~10 provider smoke를 반영함. 단 §4.2 작업표·종합 요약에 A/B/C 잔재가 있어 minor cleanup 필요",
    "SPIKE2-02": "resolved: E-provider Day 10 gate와 Vercel preview/Auth.js production-like/session persistence/403 matrix가 추가됨",
    "SPIKE2-03": "resolved: DATA_MODEL v0.24 C-23 instanceMemberships에 active·deactivatedAt·deactivatedBy가 추가됨",
    "SPIKE2-04": "resolved: REVIEW_WORKFLOW AuditAction이 signed-url-revocation-requested로 rename되고 bearer URL revoke 한계가 명시됨",
    "SPIKE2-05": "mostly_resolved: partial state matrix가 추가됨. 단 Day 10 의사결정 표에 D PROVIDER_PASS라는 불가능 상태가 남음",
    "SPIKE2-06": "accepted_for_v1: A fallback blast radius는 충분히 상세화됨. B/C/D/E는 A보다 얕지만 Day 10 운영을 막는 수준은 아님",
    "SPIKE2-07": "resolved: D/E day artifact가 명시됨"
  },
  "sot_cascade_check": {
    "INFRA_DECISIONS_DRAFT.md": "substantively_synced: §4.1과 §4.2 dependency는 5 Spike/10일·E-provider gate를 반영. §4.2 Phase 0 작업표와 종합 결정 요약에 A/B/C 잔재가 있음",
    "REVIEW_WORKFLOW.md": "synced: AuditAction에 signed-url-issued와 signed-url-revocation-requested 반영됨",
    "DATA_MODEL.md": "synced: C-23 instanceMemberships active/deactivation 필드와 매 요청 검증 설명 반영됨"
  },
  "findings": [
    {
      "id": "SPIKE3-01",
      ...
      "location": "PHASE0_WEEK1_SPIKES_DRAFT.md:427-437",
      "issue": "Day 10 의사결정 매트릭스가 `A·D PROVIDER_PASS + B·C·E LOCAL_PASS` 상태를 사용하지만, Spike D는 local-only gate로 정의되어 `D PROVIDER_PASS`가 존재하지 않습니다.",
      "impact": "acceptance 자체를 막지는 않지만, Day 10 종합 보고서에서 실제 상태 조합을 표에 매핑할 때 혼선이 생깁니다. 특히 D는 `LOCAL_PASS`만으로 Week 4 M0 migration을 unlock하는 구조라 provider 상태를 요구하면 불필요한 hold로 읽힐 수 있습니다.",
      "recommendation": "해당 행을 `A PROVIDER_PASS + D LOCAL_PASS + B LOCAL_PASS + C/E provider pending or LOCAL_PASS`처럼 실제 상태 enum에 맞게 정정하세요. D 관련 행은 `LOCAL_PASS`/`INCONCLUSIVE`만 사용하면 됩니다."
    },
    {
      "id": "SPIKE3-02",
      "severity": "minor",
      "category": "sot-cleanup",
      "location": "INFRA_DECISIONS_DRAFT.md:326-330, 436-445",
      "issue": "INFRA §4.1은 5 Spike/10일로 동기화됐지만, §4.2 Phase 0 작업표는 `Week 1 (Spike) | Spike A·B·C 진행`으로 남아 있고, 종합 결정 요약도 `Week 1 Spike A·B·C gate`라고 되어 있습니다. 또한 §4.2 heading이 두 번 사용됩니다.",
      "impact": "핵심 SoT인 §4.1과 Spike 문서는 일치하므로 blocking drift는 아닙니다. 다만 요약표만 보는 독자가 D/E를 후속 spike로 오해할 수 있습니다.",
      "recommendation": "INFRA §4.2 작업표를 `Week 1~2 Day 1~10 | Spike A·B·C·D·E`로 정정하고, 종합 결정 요약의 `A·B·C gate`를 `A·B·C·D·E 10일 gate`로 바꾸세요. 중복 §4.2 heading은 하나를 §4.3 등으로 renumbering하세요."
    }
  ],
  "new_critical_scenarios_missing": [],
  "acceptance_conditions": [
    "blocking 0개: 충족",
    "major 0~2개: 충족 — major 1개",
    "SoT cascade 동기화: 핵심 cascade 충족. INFRA 요약부 잔재는 minor cleanup",
    "솔로 + AI 보조 운영 가능: 충족 — Day artifact, provider gates, partial state matrix가 있어 운영 가능"
  ],
  "acceptance_decision": {
    "ready_for_acceptance": true,
    "recommended_version": "v1.0",
    "required_before_v1_label": [
      "SPIKE3-01 Day 10 의사결정 매트릭스의 D 상태명을 LOCAL_PASS로 정정"
    ],
    "can_fix_after_acceptance": [
      "SPIKE3-02 INFRA 작업표·종합 요약의 A/B/C 잔재와 §4.2 중복 heading cleanup"
    ]
  }
}
tokens used
