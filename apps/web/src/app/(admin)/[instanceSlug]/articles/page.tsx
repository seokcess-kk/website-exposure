// @glitzy/web/(admin)/[instanceSlug]/articles — 아티클 목록
...
export default async function ArticlesListPage({ params }: { params: { instanceSlug: string } }) {
  let pageCtx;
  try {
  ...
  try {
    rows = await withSkeletonTx(
      ...
      async (tx, ctx) => {
        ...
        return tx<Row[]>`
          SELECT a.slug, a.title, a.status::text AS status, a.risk_level::text AS risk_level,
                 d.name AS author_name, a.updated_at, a.published_at
            FROM article a
            LEFT JOIN doctor_profile d
              ON d.instance_id = a.instance_id AND d.id = a.author_doctor_id
  ...
  return (
    <main className="flex flex-col gap-6">
      <header className="flex items-center justify-between">
        ...
        <Link href={`/${params.instanceSlug}/articles/new`} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800">
          + 신규 추가
        </Link>
      </header>
            ...
            {rows.map((r) => (
              <tr key={r.slug} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2 text-right">
                  <Link href={`/${params.instanceSlug}/articles/${r.slug}`} className="text-xs text-blue-700 underline">편집</Link>
                </td>
              </tr>
            ))}
          </tbody>
