...
export default async function ArticleNewPage({ params }: { params: { instanceSlug: string } }) {
  ...
  try {
    doctorOptions = await withSkeletonTx({ signedToken: pageCtx.signedToken, instanceId: pageCtx.instanceId }, async (tx, ctx) => {
  ...
  return (
    <main className="flex flex-col gap-6">
      ...
      <ArticleForm action={bound} initial={null} isNew doctorOptions={doctorOptions} />
    </main>
  );
}
