...
export default async function ConditionEditPage({ params }: { params: { instanceSlug: string; slug: string } }) {
  ...
  try {
    const result = await withSkeletonTx(
      ...
      async (tx, ctx): Promise<{ initial: MedicalConditionInitial | null; treatmentOptions: TreatmentOption[]; isPublished: boolean }> => {
        ...
        `;
        const r = rows[0];
        if (!r) return { initial: null, treatmentOptions: [], isPublished: false };
        const treatments = await tx<{ id: string; title: string }[]>`
  ...
  return (
    <main className="flex flex-col gap-6">
      ...
      <MedicalConditionForm action={boundSave} initial={initial} isNew={false} instanceSlug={params.instanceSlug} treatmentOptions={treatmentOptions} />
