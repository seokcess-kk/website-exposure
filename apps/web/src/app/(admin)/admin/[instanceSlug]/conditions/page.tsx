// @glitzy/web/(admin)/[instanceSlug]/conditions — Conditions 목록 (EXPOSURE_READINESS Phase B)
  ...
  return (
    <main className="flex flex-col gap-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">증상 안내 페이지</h1>
          <p className="mt-1 text-sm text-slate-500">의료 검색 유입용 증상/상황 페이지 (P-007/P-008).</p>
        </div>
        <Link href={`/admin/${params.instanceSlug}/conditions/new`} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800">
          + 신규 추가
        </Link>
      </header>
            ...
            {rows.map((r) => (
              <tr key={r.slug} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2 text-right">
                  <Link href={`/admin/${params.instanceSlug}/conditions/${r.slug}`} className="text-xs text-blue-700 underline">편집</Link>
                </td>
              </tr>
            ))}
          </tbody>
