  ...
  return (
    <main className="flex flex-col gap-6">
      <header className="flex items-center justify-between">
        ...
        <Link href={`/admin/${params.instanceSlug}/treatments/new`} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800">
          + 신규 추가
        </Link>
      </header>
            ...
            {rows.map((r) => (
              <tr key={r.slug} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2 text-right">
                  <Link href={`/admin/${params.instanceSlug}/treatments/${r.slug}`} className="text-xs text-blue-700 underline">
                    편집
                  </Link>
                </td>
              </tr>
