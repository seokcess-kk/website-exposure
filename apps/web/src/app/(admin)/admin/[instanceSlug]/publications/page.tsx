  ...
  try {
    rows = await withSkeletonTx(
      ...
      async (tx, ctx) => {
        ...
        return tx<Row[]>`
          SELECT p.slug, p.title, p.journal,
            ...
            LEFT JOIN doctor_profile d
              ON d.instance_id = p.instance_id AND d.id = p.author_doctor_id
           WHERE p.instance_id = ${ctx.instanceId}::uuid
           ORDER BY p.published_date DESC, p.updated_at DESC
        `;
      },
    );
  } catch (err) {
  ...
  return (
    <main className="flex flex-col gap-6">
      <header className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">학술 인용 (Publication)</h1>
        <Link href={`/admin/${params.instanceSlug}/publications/new`} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800">
          + 신규 추가
        </Link>
      </header>
        ...
        <table className="w-full border-collapse rounded-md border border-slate-200 bg-white text-sm">
          <thead className="bg-slate-50 text-left text-xs uppercase text-slate-500">
            <tr>
              ...
              <th className="px-3 py-2">학술지</th>
              <th className="px-3 py-2">게재일</th>
              <th className="px-3 py-2">대표 의료진</th>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.slug} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2 text-xs text-slate-500">{r.journal ?? "—"}</td>
                ...
                <td className="px-3 py-2 text-right">
                  <Link href={`/admin/${params.instanceSlug}/publications/${r.slug}`} className="text-xs text-blue-700 underline">편집</Link>
                </td>
              </tr>
            ))}
          </tbody>
