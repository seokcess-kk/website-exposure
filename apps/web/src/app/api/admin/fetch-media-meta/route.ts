// @glitzy/web/api/admin/fetch-media-meta — URL → MediaAppearance 자동 채우기
// 사용자 검수 2026-05-20 — YouTube oEmbed + 일반 사이트 og:* meta scrape
// site-meta-fetch 패턴 정합
...
async function emitBestEffort(sqlBase: ReturnType<typeof getSqlBase>, input: Parameters<typeof emitAuditEvent>[1]): Promise<void> {
  try {
    ...
    console.error("[fetch-media-meta] audit emit failed", err);
  }
}
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    await emitBestEffort(sqlBase, { eventType: "fetch-media-meta-failed", reason });
  ...
  try {
    ...
    await emitBestEffort(sqlBase, { eventType: "fetch-media-meta-failed", actorUserId: userId, reason });
  ...
  try {
    ...
    if (err instanceof AuthDeniedError) {
      await emitBestEffort(sqlBase, { eventType: "fetch-media-meta-failed", actorUserId: userId, reason: err.reason });
      return NextResponse.json({ ok: false, error: "콘텐츠 편집 권한이 없습니다." }, { status: 403 });
    }
    throw err;
  }

  const meta = await fetchMediaMeta(parsed.data.url);
  await emitBestEffort(sqlBase, {
    eventType: "fetch-media-meta-ok",
    actorUserId: userId,
    payload: { source: meta.source, channelType: meta.channelType, hasTitle: meta.title !== null },
  });
