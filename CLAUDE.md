...
**현재 milestone (2026-05-27)**: 사용자 7항목 + (a)(b)(c) plan 3종 v1.0 acceptance + Option A 운영자 일상 단순화 + 후속 cycle 3 (CAI-DEFER-13 · NSI-DEFER-07 · MTL-DEFER-04) 순차 진행 中. 최신 cycle: **E2E Phase 1 smoke (Playwright)** (2026-05-27) — 운영 시작 시점 검증 자동화. `apps/web/e2e/smoke.spec.ts` 17 PASS (공개 사이트 3 + 어드민 진입 3 + entity page 9 + 신규 UI 2). `globalSetup` 안 `/[slug]/demo-admin-enter` 진입 → `glitzy_session` cookie storage state 저장 reuse. 직전: MTL-DEFER-04 — search_visibility + conversion_event 합쳐 3 신규 (low-conversion-traffic · low-ctr · naver-only-weak). today-actions 안 자동 합류 (5 readiness + 3 conversion = 8 source). 19 vitest PASS (today-actions 9 + conversion-improvements 10). 직전: NSI-DEFER-07 gap view — visibility-metrics 페이지 안 신규 섹션. query/page 단위 3 분류 (google-only · naver-only · both) + bucket count 카드 + 상위 30 표 + 분류별 hint. classifyGap pure helper 10 vitest PASS. 직전: CAI-DEFER-13 keyword secondary 추천 — apply-keyword-match 안 primary 1 + secondaries 0..N 지원 + UI radio→primary radio + secondary checkbox (mutual exclusion). 직전: ADMIN_SIMPLIFY (Option A 운영자 일상 단순화) — "기능 완성도 우선, 운영 부담 후순위" 진단 후 refactor. (a) 대시보드 최상단 "오늘 할 일" 카드 신규 (`selectTodayActions` improvement-queue 5 카테고리 top 3 · count desc + priority 동률 fallback) (b) 운영 현황 8 → 5 카드 (UnlinkedEvidence·Stale·LowReadinessPublished 3 카드 제거 — 모두 오늘 할 일 + improvement-queue deep link 안 중복) (c) NavMenu 14 항목 3 그룹화 (운영 일상 4 · 콘텐츠 7 · 정보 setup 2) + separator + tier 별 색 강약. 7 vitest PASS · 누계 251 PASS. 직전: CAI v1.1 LlmUsageCard. 잔여 = NSA v1.x (OpenAPI · gap 분석) + CAI/CCAL/MTL/NPL-DEFER 다수.
...
- **2026-05-27**: E2E Phase 1 smoke (Playwright) 합류. `apps/web/playwright.config.ts` (port 3000 default · `E2E_PORT` env override · reuseExistingServer) + `apps/web/e2e/global-setup.ts` (`/[slug]/demo-admin-enter` request 안 `glitzy_session` cookie 저장 → `e2e/.auth/storage.json` storage state reuse) + `apps/web/e2e/smoke.spec.ts` 17 시나리오 (공개 사이트 3 + 어드민 진입 + NavMenu 3 그룹 14 항목 + TodayActionsCard + entity page 9 + visibility source tab + improvement-queue heading). 첫 실행 안 fresh chromium context cookie 없음 → admin sign-in redirect loop 발견 → globalSetup 패턴 적용 후 17/17 PASS (2.9분). `test:e2e` / `test:e2e:ui` script 추가. `.gitignore` 안 test-results / playwright-report 등 + `e2e/.gitignore` 안 `.auth/` 제외. dev mode timeout 90s test / 60s navigation (Next.js dev compile 시간 확보).
- **2026-05-27**: MTL-DEFER-04 — improvement-queue 안 conversion 카테고리 합류. `lib/admin/conversion-improvements.ts` (loadConversionImprovements + 3 카테고리 + env threshold override · 4 신규 env). search_visibility_snapshot 의 page_url 별 source 별 합산 + conversion_event 합산 + positionUnavailable=true row 안 avg_position 합산 제외 (NSA sentinel 정합). 3 카테고리 정의: low-conversion-traffic (total clicks ≥ 10 + conversion 0) · low-ctr (네이버 avg position ≤ 10 + impressions ≥ 5 + naver clicks 0) · naver-only-weak (Google 0 + naver imp < 20). conversion 매칭 — svs.page_url 안 host prefix 제거 후 conversion_event.page_path 와 매칭. `today-actions.ts` 시그니처 변경 (`selectTodayActions(queue, conversion, slug)` · TodayActionCategory union 안 3 신규 추가). dashboard page 안 Promise.all 6번째 RTT 추가. improvement-queue page 안 ConversionSection 3 신규 (toneClass 별 색 분리). 19 vitest PASS (today-actions 9 카테고리 mix · 모든 anchor mapping + conversion-improvements 10 threshold/topLimit/host prefix 매칭). 누계 273 PASS. CAI-DEFER-04 정합 — 본 cycle 안 conversion 신호 자동 noise 회피 (totalItems > 0 시만 ConversionSection 노출).
- **2026-05-27**: NSI-DEFER-07 — GSC ↔ NSA gap 분석 view 합류. `lib/admin/search-visibility-gap.ts` (loadVisibilityGap + classifyGap pure helper). query/page 별 google_imp/naver_imp aggregate 후 3 분류 (google-only · naver-only · both) — google-search-console source = google · naver-searchadvisor = naver · bing-webmaster source 는 v1 안 무시 (별 marker). 0/0 row 자동 제외. `/visibility-metrics` 페이지 안 GapSection 컴포넌트 (3 bucket 카드 + query/page tab + 상위 30 표 + 각 분류 actionable hint). 10 vitest PASS (classify 분기 + load mock tx + topLimit + bing 무시 + 0 row 제외). 누계 +10 PASS. typecheck/build PASS.
- **2026-05-27**: CAI-DEFER-13 — keyword secondary 추천 합류. AI 추천 modal 안 radio 1개 (primary) → primary 1 (radio) + secondary 0..N (checkbox · primary row 안 자동 disable + mutual exclusion useEffect). `applyKeywordMatchAction` 시그니처 변경: `input.primary` + `input.secondaries[]` (server 안 dedup). primary UPSERT (is_primary=true · relevance 70) + secondaries UPSERT (is_primary=false · relevance 50 · ON CONFLICT 안 기존 primary 보존). 모든 entity readiness 재계산. accept label dynamic ("primary 1 연결" vs "primary 1 + secondary N 연결"). typecheck PASS · build PASS. CAI-DEFER 잔여 12 (DEFER-13 해소).
- **2026-05-27**: ADMIN_SIMPLIFY (Option A 운영자 일상 단순화) — "기능 완성도 우선, 운영 부담 후순위" 진단 후 refactor. 신규 entity X · DB 변경 X · 순수 UI/lib refactor. (a) `lib/admin/today-actions.ts` 안 `selectTodayActions(queue, slug)` pure function — improvement-queue 5 카테고리 (low-readiness · evidence-missing · seo-improve · stale · relations-thin) count desc 정렬 + priority 동률 fallback (low-readiness > evidence-missing > seo-improve > stale > relations-thin) + top 3 + anchor href 생성. (b) `TodayActionsCard.tsx` 신규 (대시보드 최상단 · ol 3 action + 0 row 안 healthy 안내 + footer 안 affectedEntityCount/healthyCount). (c) `VisibilityOverviewSection` 안 UnlinkedEvidenceCard · StaleContentCard · LowReadinessPublishedCard 3 함수 제거 (모두 improvement-queue deep link 중복 · 신호 분산 회피) — 5 카드 유지 (KeywordCoverage · AverageReadiness · JsonLdDefect · ConversionTraffic · LlmUsage). (d) `NavMenu.tsx` 14 항목 3 그룹화 (ops 4 · content 7 · setup 2) · separator + GroupSegment + navItemClass — ops = brand-primary tier (대시보드·개선큐·검색노출·캘린더) · content = neutral tier · setup = neutral 약 tier. hide X (사용자 혼란 회피). (e) dashboard page Promise.all 안 loadImprovementQueue 합류 (4 RTT → 5 RTT 1회 추가 · 5번째 RTT 만 추가). 7 vitest PASS (정렬 + 동률 priority + 5 카테고리 anchor mapping + instanceSlug interpolation + 0 input). 누계 251 PASS · pre-existing slugify SLG-02 1 fail 무관. typecheck PASS · web:build PASS.
